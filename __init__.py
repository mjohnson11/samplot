...
def get_read_data(
    ...
    max_coverage = 0
    index_fname_counter = -1
    for bam_file_name in bams:
        index_fname_counter += 1
        bam_file = None
        try:
            if not reference:
                bam_file = pysam.AlignmentFile(bam_file_name, "rb", index_filename=index_filenames[index_fname_counter])
            else:
                bam_file = pysam.AlignmentFile(
                    bam_file_name, "rc", index_filename=index_filenames[index_fname_counter], reference_filename=reference
                )
        except Exception as err:
            print("Error:", err, file=sys.stderr)
            sys.exit(1)
...
def bam_index_finder(bam, bam_to_bai=False):
        if not os.path.isfile(bam):
            print("alignment file {} does not exist or is not a valid file".format(bam))
        if bam_to_bai:
            idx_file = bam.replace('.bam', '.bai')
        else:
            ...
            if ext not in options:
                print("alignment file {} is not in SAM/BAM/CRAM format".format(bam))
            ...
            if not os.path.isfile(bam + "." + idx_type):
                ...
                if not os.path.isfile(bam + "." + idx_type):
                    print("alignment file {} has no index".format(bam))
            return bam + "." + idx_type

def check_gff_file(transcript_file):
    if not os.path.isfile(transcript_file):
        print.error("transcript file {} does not exist or is not a valid file".format(transcript_file))
    options = ["gff", "gff3"]
    ...
    ext = ext.lower()
    if ext not in options:
        print.error("transcript file {} is not in GFF/GFF3 format".format(transcript_file))

    idx_file = transcript_file + ".tbi"
    if not os.path.isfile(idx_file):
        print("transcript file {} is missing .tbi index file".format(transcript_file))

def check_bed_file(annotation_file):
    if not os.path.isfile(annotation_file):
        print("annotation file {} does not exist or is not a valid file".format(annotation_file))
    ...
    ext = ext.lower()
    if ext != "bed":
        print("annotation file {} is not in BED format".format(annotation_file))

    idx_file = annotation_file + ".tbi"
    if not os.path.isfile(idx_file):
        print("annotation file {} is missing .tbi index file".format(annotation_file))

########################################################################
# main block
########################################################################
def plot(bams, output_file, start, end, chrom, index_files_bam_to_bai=False, annotation_files=None, annotation_filenames=None, coverage_tracktype=None, plot_height=None, plot_width=None, include_mqual=1, separate_mqual=1, json_only=False, start_ci=None, end_ci=None, long_read=1000, ignore_hp=None, min_event_size=20, xaxis_label_fontsize=6, yaxis_label_fontsize=6, legend_fontsize=6, annotation_fontsize=6, common_insert_size=False, hide_annotation_labels=False, coverage_only=False, same_yaxis_scales=False, marker_size=3, dpi=300, zoom=500000, debug=False, transcript_file=None, transcript_filename=None, max_coverage_points=1000, window=-1, max_depth=1, sv_type=None, titles=None, reference=None, z=4):
    """
    To support translocations, the SVs are specified as an array of 
    genome_interval. For now we let that arry be size 1 or 2.
    # Required args: list of bam files, output file, start, end, chrom
    # optional args are the same as command line samplot, but I've added index_files_bam_to_bai,
    # which if True assumes the index files are like the .bam files but with .bai in place of .bam (instead of .bam.bai)
    """
    if window == -1:
        window = 0.5 * (end - start)

    # Doing some checking that usually was done in the parser
    bam_index_files = [bam_index_finder(bam, index_files_bam_to_bai) for bam in bams]
    if transcript_file:
        check_gff_file(transcript_file)
    if annotation_file:
        check_bed_file(annotation_file)

    if (annotation_files 
            and annotation_filenames 
            and len(annotation_files) != len(annotation_filenames)):
        print("annotation filenames do not match annotation files")
        sys.exit()

    for bam in bams:
        if ".cram" in bam:
            if not reference:
                print("Error: Missing reference for CRAM")

    if len(chrom) != len(start) != len(end):
        print("The number of chromosomes, starts, and ends do not match.")
        sys.exit()

    sv = []
    for i in range(len(chrom)):
        chrom[i] = chrom[i].replace("chr", "")
        sv.append(genome_interval(chrom[i], start[i], end[i]))
    # set up plot
    ...
    # plot legend
    plot_legend(fig, legend_fontsize, marker_size)
